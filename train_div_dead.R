...
for(i in 1:10){
	...
	d = sample(1:halfdead,halfhealth,replace=T)
	dead1 = dead[d,]
	print(table(dead1$V45))
	d = sample((halfdead+1):deadlen,halfhealth,replace=T)
	dead2 = dead[d,]
	print(table(dead2$V45))

	len = nrow(health)
	select = sample(1:len,len/2)
	train = health[select,]
	train = rbind(train,dead1)
	#dim(train)
	print(table(train$V45))

	test = health[-select,]
	test = rbind(test,dead2)
	#dim(test)
